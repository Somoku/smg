...
mod tests {
    ...
    fn test_token_mismatch_at_position() {
        let expected = [1, 2, 3, 4, 5];
        let actual = [1, 2, 99, 4, 5];

        let mismatch_pos = expected.iter().zip(actual.iter()).position(|(e, a)| e != a);
    ...
    fn test_append_only_violation_detection() {
        let prev = [1, 2, 3, 4, 5];
        let violating = [1, 2, 3, 5, 6]; // Token 4 missing

        let is_append_only = prev.len() <= violating.len() && violating[..prev.len()] == prev[..];
    ...
    fn test_deletion_violation() {
        let prev = [1, 2, 3, 4, 5];
        let shorter = [1, 2, 3]; // Deleted 4, 5

        let is_append_only = prev.len() <= shorter.len() && shorter[..prev.len()] == prev[..];
    ...
    fn test_reordering_violation() {
        let prev = [1, 2, 3, 4, 5];
        let reordered = [1, 3, 2, 4, 5, 6]; // Positions 1 and 2 swapped

        let is_append_only = prev.len() <= reordered.len() && reordered[..prev.len()] == prev[..];
    ...
    fn test_valid_append() {
        let prev = [1, 2, 3];
        let appended = [1, 2, 3, 4, 5, 6];

        let is_append_only = prev.len() <= appended.len() && appended[..prev.len()] == prev[..];
    ...
    fn test_multiple_position_mismatches() {
        ...
        assert_eq!(mismatch_positions, [1, 3, 6]);
    }
    ...
    fn test_logprob_validation() {
        ...
        let logprobs = [0.95, 0.87, 0.92, 0.78, 0.99];

        let all_valid = logprobs.iter().all(|&p| (0.0..=1.0).contains(&p));
        assert!(all_valid, "All logprobs should be valid");

        // Negative or >1.0 should be invalid
        let invalid_logprobs = [0.95, 1.5, -0.1];
        let all_invalid_valid = invalid_logprobs.iter().all(|&p| (0.0..=1.0).contains(&p));
    ...
    fn test_finish_reason_validation() {
        let valid_reasons = ["stop", "length", "tool_calls", "error"];
        let all_valid = valid_reasons.iter().all(|r| !r.is_empty());
        assert!(all_valid, "All finish reasons should be non-empty");

        let invalid_reason = "";
        assert!(
            invalid_reason.is_empty(),
            "Empty finish reason should be invalid"
        );
    }
    ...
    fn test_prefix_match_detection() {
        let prefix = [1, 2, 3, 4];
        let full_seq = [1, 2, 3, 4, 5, 6, 7];

        let is_prefix = prefix.len() <= full_seq.len() && full_seq[..prefix.len()] == prefix[..];
    ...
    fn test_prefix_mismatch_detection() {
        let expected_prefix = [1, 2, 3, 4];
        let actual_seq = [1, 2, 99, 4, 5, 6];
    ...
    fn test_empty_sequence_validation() {
        ...
        let is_append_only =
            empty.len() <= non_empty.len() && non_empty[..empty.len()] == empty[..];
    ...
    fn test_large_sequence_validation() {
        ...
        let is_append_only =
            subset.len() <= large_seq.len() && large_seq[..subset.len()] == subset[..];
        assert!(is_append_only);
    }
    ...
    fn test_token_value_ranges() {
        let tokens = [0, 1, 100, 1000, 50000, 128000]; // Typical tokenizer ranges
    ...
    fn test_mismatch_detection_consistency() {
        ...
        // First check
        let mismatch1 = expected.iter().zip(actual.iter()).position(|(e, a)| e != a);

        // Second check (should be identical)
        let mismatch2 = expected.iter().zip(actual.iter()).position(|(e, a)| e != a);

        assert_eq!(
            mismatch1, mismatch2,
            "Mismatch detection should be deterministic"
        );
    }
}
