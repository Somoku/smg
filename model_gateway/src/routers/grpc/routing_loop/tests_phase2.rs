...
mod tests {
    ...
    async fn test_version_entry_creation_on_first_sync() {
        ...
        // No entry initially
        assert!(!runtime.instance_to_version_after_sync.contains_key(&instance));
        
        // Insert first version
        runtime.instance_to_version_after_sync.insert(instance.clone(), 1);
    ...
    async fn test_version_increments_on_sync() {
        ...
        for version in 1..=5 {
            runtime.instance_to_version_after_sync.insert(instance.clone(), version);
            let current = runtime
                .instance_to_version_after_sync
                .get(&instance)
                .map(|r| *r.value());
    ...
    async fn test_independent_version_tracking_per_instance() {
        ...
        runtime.instance_to_version_after_sync.insert(inst_a.clone(), 10);
        runtime.instance_to_version_after_sync.insert(inst_b.clone(), 5);
        runtime.instance_to_version_after_sync.insert(inst_c.clone(), 7);
        
        let v_a = runtime.instance_to_version_after_sync.get(&inst_a).map(|r| *r.value());
        let v_b = runtime.instance_to_version_after_sync.get(&inst_b).map(|r| *r.value());
        let v_c = runtime.instance_to_version_after_sync.get(&inst_c).map(|r| *r.value());
    ...
    async fn test_version_entry_deletion_and_reinsertion() {
        ...
        runtime.instance_to_version_after_sync.insert(instance.clone(), 10);
        let version = runtime
            .instance_to_version_after_sync
            .get(&instance)
            .map(|r| *r.value());
    ...
    async fn test_version_transition_ordering() {
        ...
        runtime.instance_to_version_after_sync.insert(instances[0].clone(), 3);
        runtime.instance_to_version_after_sync.insert(instances[1].clone(), 5);
        
        // Filter: keep only workers >= version_tag 4
        let version_tag = 4;
        let passing = instances.iter().filter(|inst| {
            runtime
                .instance_to_version_after_sync
                .get(inst)
    ...
    async fn test_many_concurrent_instances() {
        ...
        for worker_id in 0..50 {
            ...
            let handle = task::spawn(async move {
                ...
                rt.instance_to_version_after_sync.insert(instance.clone(), (worker_id + 1) as i64);
                instance
            });
            handles.push(handle);
        }
    ...
    async fn test_version_can_regress_on_reinsertion() {
        ...
        let version = runtime
            ...
            .get(&instance)
            .map(|r| *r.value());
        assert_eq!(version, Some(5), "Version can regress (no enforcement on insertion)");
    }
    ...
    async fn test_large_version_numbers() {
        ...
        runtime.instance_to_version_after_sync.insert(instance.clone(), large_version);
